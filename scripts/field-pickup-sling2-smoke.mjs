...
try {
  ...
  snapshot = await page.evaluate(() => window.__topdownExtractionAgentApi.stageRaidWeaponLoot({ weaponId: "smg" }));
  ...
  for (let attempt = 0; attempt < 10; attempt += 1) {
    ...
    if (snapshot.raid?.player.activeWeaponSlotId === "secondary") {
      break;
    }
  }

  assert(snapshot.raid, "Raid ended before field weapon pickup could complete.");
  assert(snapshot.raid.player.activeWeaponSlotId === "secondary", `Expected Sling 2 active, got ${snapshot.raid.player.activeWeaponSlotId}`);
  assert(snapshot.raid.player.weaponId === "smg", `Expected picked SMG active, got ${snapshot.raid.player.weaponId}`);
