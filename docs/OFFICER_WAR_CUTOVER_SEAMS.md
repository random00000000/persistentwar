...
2. **Town + camps exist even if soldiers are still raid-centric**
   ...
   - Raid start also seeds Russian `camp-garrison` friendly combatants inside the right-side player camp and a Ukrainian frontline patrol near the first-town contact line. Friendly and hostile AI fire through the reusable `spawnAiWeaponProjectiles` wrapper, which feeds the same `spawnWeaponProjectiles` / `BulletState` path used by player bullets.
